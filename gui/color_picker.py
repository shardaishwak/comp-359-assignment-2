...
def show_color_picker(root):
    ...
    choose_color_button = tk.Button(color_frame, text="Pick a Color", command=lambda: choose_color(color_display_frame, next_button))
    choose_color_button.pack(pady=20)

    next_button = tk.Button(color_frame, text="Next", state="disabled", command=lambda: show_next_step(root, color_frame))
